  ...
  const handleGenerateBots = async () => {
    ...
    try {
      ...
      if (insertedPools && insertedPools.length > 0) {
        const predictionsToInsert: any[] = [];
        insertedPools.forEach((pool, poolIdx) => {
          const assignedCombo = assignedCombinations[poolIdx] || {};
...
{/* 8.4 ADMIN: GESTIÓN DE CUENTAS BANCARIAS (Pestaña "admin-bank") */}
        ...
        {activeTab === 'admin-bots' && isAdmin && (() => {
          ...
          const humanPoolsCount = allPoolsForMatchday.filter(p => humanParticipantIds.has(p.participant_id)).length;
                ...
                {(() => {
                  ...
                  const rankedBots = approvedPools
                    .map((pool: any, idx: number) => {
                      ...
                      const isBot = phone === 'BOT-0000' || (typeof refCode === 'string' && (refCode.startsWith('BOT-') || refCode.startsWith('BT-') || refCode.startsWith('REF-')));
                  ...
                  const filteredBots = rankedBots.filter(b => 
                    ...
                    (b.cloneOf && b.cloneOf.toLowerCase().includes(botSearchQuery.toLowerCase()))
                  );
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.88rem' }}>
                          <thead>
                            <tr style={{ background: 'rgba(0,0,0,0.4)', textAlign: 'left', borderBottom: '1px solid var(--border-color)' }}>
                              <th style={{ padding: '10px 12px', textAlign: 'center', width: '90px' }}>Ranking</th>
                              <th style={{ padding: '10px 12px' }}>Bot / Folio</th>
                              <th style={{ padding: '10px 12px' }}>Procedencia / Clon</th>
                              <th style={{ padding: '10px 12px', textAlign: 'center', width: '100px' }}>Puntuación</th>
          ...
          <div style={{
            ...
            {viewBotModal.cloneOf ? (
              ...
              <div style={{
                ...
                🤖 Generado mediante matriz probabilística de momios y favoritos.
              </div>
            )}
